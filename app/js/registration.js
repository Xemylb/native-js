var appStore = window.localStorage, loginsArr, userNamesArr, passArr, userSurnameArr, userAgeArr,
...
window.onload = function () {
    ...
    btn.onclick = function () {
        ...
        //разбиваем имеющиеся данный пользователей
        ...
        if (login.trim() !== '' && pass.trim() !== '' && userName.trim() !== '' && userAge.trim() !== '' && userSurname.trim() !== '') {
            if (pass == repeatPass){//проверка на правильность ввода повторного пароля
                if (loginsArr[0] == '') {//если нет ни одного зарегистрированного пользователя, создаем как бы базу
                    addUserData('empty');
                } else {
                    var checkLogFunc = function () {//если естьп проверям на совпаления логинов
                    ...
                    if (checkLogFunc() == 'copy') {
                        regError('loginCopy');
                    } else {//добовляемнового пользовтеля
        ...
        function addUserData(firstElemValue) { //функция создание как бы базы и добовления пользователя
...
function regError(errorClass) {//функция ошибок
